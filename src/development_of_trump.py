...
## Task
- Train an unsupervised classifier as an LDA model on the twitter data to extract structured information that can provide insight into topics in Trumps tweets.
- Output is in the form of an html file containing the topics (as well as a print in the terminal) and a png file for the development of topics. Both can be found in the folder output.
"""
...
ap.add_argument("-f", "--filepath", 
                default = "../data/Trump_tweets.csv", 
                help= "Path to the csv-file")

# filename of output development plot
ap.add_argument("-o", "--output_filename", 
                default = "trumps_development.png", 
                help = "Name of output file")

# number of topics
ap.add_argument("-n", "--n_topics", 
                default = 10, 
                help = "Number of topics")

# word type for part of speech tagging
ap.add_argument("-w", "--word_type", 
                default = "['NOUN', 'VERB']", 
                help = "Type of word. Choose between: 'NOUN','VERB','ADJ','ADV'")
...
def main(args):
    ...
    Trump = Tweet_development(input_file = filepath, 
                              output_file = out_file, 
                              n_topics = n_topics, 
                              word_types = word_types)
    
    # read csv and prepare for classification
    Trump.load_and_prepare()
    ...
    Trump.development_and_outputs(id2word = id2word, 
                                  corpus = corpus, 
                                  lda_model = lda_model)
    
    # print done
    print("\n[INFO] Good job! The script is now done. Have a nice day!")
...
class Tweet_development:
    ...
    def process_data(self):
        ...
        print("\n[INFO] Lemmatizing the data and doing POS tagging. This takes a few minutes...")
        # Processing the data using Ross' lda utils function
        data_processed = lda_utils.process_words(self.tweets,
                                                 nlp, 
                                                 bigram_mod,
                                                 trigram_mod,
    ...
    def lda_model(self, data_processed):
        ...
        print(f"\n[INFO] Building the LDA model. You have {len(data_processed)} tweets in the data so it might take a while")
        # Build LDA model
        ...
        print('\n[INFO] Perplexity: ', lda_model.log_perplexity(corpus))  # a measure of how good the model is. lower the better.
        ...
        print('\n[INFO] Coherence Score: ', coherence_lda)
    ...
    def development_and_outputs(self, id2word, corpus, lda_model):
        ...
        '''
        print("\n[INFO] Creating visualizations and saving as html and png in output folder")
        directory_name = os.path.join("..", "output")
        create_dir(directory_name)
        # Make gensim LDA visualization
        vis = pyLDAvis.gensim.prepare(lda_model, corpus, dictionary=lda_model.id2word)
        # define output path and filename
        out_path = os.path.join(directory_name, "lda_topics.html")
        # save visualization as html
        pyLDAvis.save_html(vis, out_path)
        print("\n[INFO] Gensim LDA visualization is saved as", out_path)
        ...
        out_path = os.path.join(directory_name, self.output_file)
        ...
        print("\n[INFO] Lineplot for development of topics in tweets is saved as", out_path)

def create_dir(directory_name):
        '''
        Creates a directory if it doesn't exist.
        '''
        path2dir = os.path.join(directory_name)
        if not os.path.exists(path2dir):
            os.mkdir(path2dir)
            print("\n[INFO] Directory " , path2dir ,  " Created ")
        else:
            print("\n[INFO] Directory " , path2dir ,  " already exists")
